...
import { useConnectionFeeReport } from '@/hooks/useConnectionFeeReport';
...
export default function ConnectionFeeReport() {
  const { data: records = [], isLoading } = useConnectionFeeReport();
  ...
  const totalConnectionFee = useMemo(() => {
    return filteredRecords.reduce((sum, record) => sum + (record.connection_fee || 0), 0);
  }, [filteredRecords]);
  ...
  const handleExportCSV = () => {
    const headers = ['Date', 'Created By', 'Username', 'Full Name', 'Connection Fee'];
    ...
    a.href = url;
    a.download = `connection-fee-report-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    a.click();
            ...
            <tbody>
              ${filteredRecords.map(record => `
                <tr>
                  ...
                  <td class="text-right">৳${(record.connection_fee || 0).toLocaleString()}</td>
                </tr>
              `).join('')}
              <tr class="total-row">
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(14);
    doc.setTextColor(0);
    doc.text(`Total Connection Fee: ${formatPDF(totalConnectionFee)}`, 20, 45);
    doc.setFontSize(11);
    doc.setTextColor(100);
    ...
    doc.text('Created By', 50, yPos);
    doc.text('Username', 90, yPos);
    doc.text('Connection Fee', 145, yPos);
    ...
    filteredRecords.forEach((record) => {
      ...
      doc.text((record.creator_name || '-').slice(0, 18), 50, yPos);
      doc.text(record.username.slice(0, 18), 90, yPos);
      doc.text(formatPDF(record.connection_fee || 0), 145, yPos);
      yPos += 7;
    });
    ...
    doc.text(formatPDF(totalConnectionFee), 145, yPos);
    
    doc.save(`connection-fee-report-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
  };

  return (
    <DashboardLayout title="Connection Fee Report" subtitle="View all connection fee collections">
      {/* Summary Card */}
                    ...
                    <>
                      {filteredRecords.map((record) => (
                        <TableRow key={record.id}>
                          ...
                          <TableCell>{record.creator_name || '-'}</TableCell>
                          <TableCell>{record.username}</TableCell>
                          <TableCell>{record.full_name || '-'}</TableCell>
                          <TableCell className="text-right font-medium">
                            ৳{(record.connection_fee || 0).toLocaleString()}
                          </TableCell>
                        </TableRow>
                      ))}
                      {/* Total Row */}
