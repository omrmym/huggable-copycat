...
import { Loader2, DollarSign, Users, UserX, Wallet, Search } from 'lucide-react';
import { ResellerRechargeDialog } from '@/components/reseller/ResellerRechargeDialog';

export default function ResellerBillingStatistics() {
  ...
  useEffect(() => {
    ...
    if (paid === 'true') setPaidFilter('paid');
    if (billing === 'auto_renew') setPaidFilter('auto_renew');
  }, [searchParams]);

  // Fetch users via edge function
  const { data: users = [], isLoading: usersLoading } = useQuery({
    queryKey: ['reseller-billing-users', reseller?.id, isSuperAdmin],
    queryFn: async () => {
  ...
  const { data: autoGeneratedTransactions = [], isLoading: autoTxLoading } = useQuery({
    queryKey: ['reseller-auto-generated-transactions', reseller?.id, isSuperAdmin],
    queryFn: async () => {
      const sessionToken = localStorage.getItem('reseller_session_token');
      const { data, error } = await supabase.functions.invoke('reseller-get-transactions', {
        body: { resellerId: reseller?.id, isSuperAdmin, isAutoGenerated: true, session_token: sessionToken },
      });
      if (error) throw error;
  ...
  const StatCard = ({ title, value, icon: Icon, subtitle }: {
    ...
    <Card className="bg-card border-border">
      ...
      <CardContent>
        <div className="text-2xl font-bold">{usersLoading ? <Skeleton className="h-8 w-20" /> : value}</div>
  ...
  return (
    <ResellerLayout
      title={isSuperAdmin ? "All Billing Statistics" : "Billing Statistics"}
      subtitle="Overview of billing and payment metrics"
    >
      {/* Summary Stats */}
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          {(paidFilter === 'auto_renew' ? autoTxLoading : usersLoading) ? (
                  ...
                  {autoGeneratedTransactions.length === 0 ? (
                    ...
                    autoGeneratedTransactions.map((tx: any) => (
                      <TableRow key={tx.id}>
                        <TableCell>
                          <button
                            onClick={() => tx.radius_user_id && navigate(`/reseller/users/${tx.radius_user_id}`)}
