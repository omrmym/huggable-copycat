...
import { Search, ArrowLeft, Wifi, WifiOff, Eye } from 'lucide-react';

export default function OnlineOfflineUsers() {
  ...
  const isLoading = onlineLoading || usersLoading || areasLoading;
  ...
  const onlineUsers = useMemo(() => {
    return allUsers.filter(user => onlineUsernames.includes(user.username));
  ...
  const filteredOnlineUsers = useMemo(() => {
    ...
    if (selectedAreaId !== 'all') {
      filtered = filtered.filter(user => user.area_id === selectedAreaId);
    }
  ...
  if (isLoading) {
    return (
      <DashboardLayout title="Online / Offline Users" subtitle="View users by connection status">
      ...
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="online" className="mt-4">
            <UserTable users={currentUsers} type="online" areaMap={areaMap} onViewUser={(id) => navigate(`/users/${id}`)} />
          </TabsContent>

          <TabsContent value="offline" className="mt-4">
            <UserTable users={currentUsers} type="offline" areaMap={areaMap} onViewUser={(id) => navigate(`/users/${id}`)} />
          </TabsContent>
        </Tabs>
      </div>
    </DashboardLayout>
